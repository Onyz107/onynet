...
func BenchmarkManager_Accept(b *testing.B) {
	...
	go func() {
		for {
			clientManager.OpenStream("testStream", context.Background(), 0)
		}
	}()

	b.ResetTimer()
	for b.Loop() {
		serverManager.AcceptStream("testStream", context.Background(), 0)
	}
}
...
func BenchmarkStream_Send(b *testing.B) {
	...
	go func() {
		stream, err := serverManager.AcceptStream("benchmarkSend", context.Background(), 0)
		if err != nil {
			b.Error(err)
			return
		}
	...
	clientStream, err := clientManager.OpenStream("benchmarkSend", context.Background(), 0)
	if err != nil {
		b.Fatal(err)
	}
